...
export async function extractJD(jobDescription: string, retryCount: number = 5): Promise<any> {
  try {
    ...
    if (retryCount > 1) {
      return extractJD(jobDescription, retryCount - 1);
    } else {
      throw new Error("Failed to extract job description after several attempts.");
    }
  }
}
